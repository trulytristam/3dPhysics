...
impl ConstraintDesc {
    fn get_correction(&self, a: &mut Object, b: &mut Object)->Vector3<f64>{
        let diff_current_ab = (a.o*UnitQuaternion::new(V3::new(0.,PI/2.,0.))) * b.o.inverse();
...
impl Constraint {
    fn solve_constraing_angular(&mut self, o1: &mut Object, o2: &mut Object, h: f64){
        ...
        //println!("angular correction vector : {:?}",nvec);
        //println!("angular correction vector length: {:?}",nvec.norm());
        //println!("correction vector: dir: {:?}",nvec.normalize());
        //println!("angular compliance: {:?} for timestep {}",a, h*h);
        //println!("angular_lagrange: {:?}",self.angular_lagrange);

        //println!("correction scale: {:?}",dy);
        //println!("generalized inverse masses {:?} , {:?}", w1,w2);
        //println!("top {:?}",top);
        //println!("bot {:?}",bot);
        //println!("aq: {:?}",aq);
        //println!("bq: {:?}",bq);
        //println!("a ii_t: {:?}",o1.ii_t);
        //println!("b ii_t: {:?}",o2.ii_t);
        //println!("p: {:?}",p);
        //println!("rhs: {:?}",(self.c_desc.apoint.cross(&p1_local)));
