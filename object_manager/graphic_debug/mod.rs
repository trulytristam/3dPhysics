...
impl GraphicDebug {
    ...
    pub fn getlines(
        &self,
        dim: (f64, f64),
        cam: (Vector3<f64>, UnitQuaternion<f64>),
    ) -> [f64; 1024] {
        let mut out = [0.; 1024];
        let mut j = 0;
    ...
    pub fn debug_constraint(&mut self, c: &Constraint, objects: &Vec<Object>) {
        if c.a >= objects.len() as u32 || c.b >= objects.len() as u32 {
            return;
        }
        ...
        self.addline(
            objects[c.a as usize].localtoglobal(loca),
            objects[c.b as usize].localtoglobal(locb),
            V3::new(1., 0.7, 0.),
        );
    }
    ...
    pub fn add_set_of_points_as_connected_lines(
        &mut self,
        p: Vec<Vector3<f64>>,
        color: Vector3<f64>,
    ) {
