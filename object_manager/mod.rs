...
impl ObjectManager {
    pub fn new() -> Self {
        ...
        let mut om = ObjectManager {
            cam_speed: 20.,
            input_manager: InputManager::new_default(),
            lights: vec![],
            physics_manager: PhysicsManager::new(),
            objects: o,
            cam: (V3::new(0., 0., -1.), UnitQuaternion::default()),
            debug: d,
            screen_dim: (0., 0.),
        ...
        om.physics_manager.constraints[0]
            .c_desc
            .generate_default_joint_axis_two();
...
//       if self.input_manager.mouse.hasmoved {
//           let axisx = UnitVector3::new_normalize(cx);
//           let axisy = UnitVector3::new_normalize(V3::new(0.,1.,0.));
//           self.cam.1 = UnitQuaternion::from_axis_angle(
//               &axisx,
//               self.input_manager.mouse.delta.y*self.cam_speed * deltatime * 0.05,
//           ) * self.cam.1;
//           self.cam.1 = UnitQuaternion::from_axis_angle(
//               &axisy,
//               self.input_manager.mouse.delta.x*self.cam_speed * deltatime * 0.05,
//           ) * self.cam.1;
//       }
        if self.input_manager.get_key_held(Key::UP) {
            self.cam.1 = UnitQuaternion::from_axis_angle(
                &UnitVector3::new_normalize(cx),
        ...
        if self.input_manager.get_key_held(Key::LEFT) {
            self.cam.1 = UnitQuaternion::from_axis_angle(
                &UnitVector3::new_normalize(V3::new(0., 1., 0.)),
                self.cam_speed * -deltatime * 0.15,
            ) * self.cam.1;
        }
    ...
    pub fn add_object(&mut self, p: V3, s: BasicShape, bstatic: bool, d: f64, a: V3) {
        match s {
            BasicShape::Cube(dim) => {
                self.objects
                    .push(Object::new(p, BasicShape::Cube(dim), bstatic, d, a))
            }
            BasicShape::Pyramid => {}
            BasicShape::Sphere(r) => {}
        }
...
//        self.add_debug_lines_for_cube(1, V3::new(1., 0., 1.));
    ...
    pub fn render(
        ...
        .unwrap();
        let ub_pos =
            glium::uniforms::UniformBuffer::new(display, self.get_object_position()).unwrap();
        // println!("frself main: {:?}", OM.getObjectOrientations()[0] );
        let ub_o =
            glium::uniforms::UniformBuffer::new(display, self.get_object_orientations()).unwrap();
        let debug_line_colors =
            glium::uniforms::UniformBuffer::new(display, self.debug.get_line_colors()).unwrap();
        let ub_o_dim =
            glium::uniforms::UniformBuffer::new(display, self.get_object_dims()).unwrap();
        let lights_buffer =
    ...
    fn get_debug_program(&mut self, display: &glium::Display) -> glium::VertexBuffer<VertexLine> {
        ...
        for i in 0..256 {
            ...
            let j = i * 4;
            debuglines.push(VertexLine {
                position: [lines[j] as f32, lines[j + 1] as f32],
                color: [
                    linecolors[i * 3] as f32,
                    linecolors[i * 3 + 1] as f32,
                    linecolors[i * 3 + 2] as f32,
                ],
            });
            debuglines.push(VertexLine {
                position: [lines[j + 2] as f32, lines[j + 3] as f32],
                color: [
                    linecolors[i * 3] as f32,
                    linecolors[i * 3 + 1] as f32,
                    linecolors[i * 3 + 2] as f32,
