...
pub trait HasSupport{
    fn get_support(&self, dir: V3)->V3{
        V3::default()
    }
}
impl HasSupport for Collider {
    fn get_support(&self,dir: V3) -> V3 {
...
impl HasSupport for V3{
    fn get_support(&self,dir: V3)-> V3{
        self.clone()
    }
}
fn get_cso_support<T: HasSupport>(first: &T, other: &T, dir: V3) -> V3 {
    let a = first.get_support(dir);
...
pub fn triangle_closest(i: &V3, j: &V3, k: &V3) -> (V3, SType) {
    ...
    match areas {
        (true, true, true) => (
            v * i + w * j + u * k,
            SType::Face(i.clone(), j.clone(), k.clone()),
        ),
        (true, true, false) => line_closest(k, ki, kin),
        (true, false, false) => (k.clone(), SType::Vert(k.clone())),
        (true, false, true) => line_closest(j, jk, jkn),
        (false, true, true) => line_closest(i, ij, ijn),
        (false, true, false) => (i.clone(), SType::Vert(i.clone())),
        (false, false, false) => (j.clone(), SType::Vert(j.clone())),
        (false, false, true) => (j.clone(), SType::Vert(j.clone())),
    }
}
...
impl Simplex {
    ...
    pub fn closest(&self) -> (V3, SType) {
        match self {
            ...
            Simplex::Thr(p, p2, p3) => triangle_closest(p, p2, p3),
            Simplex::Fou(p1, p2, p3, p4) => simplex_closest(p1, p2, p3, p4),
        }
    }
...
pub fn gjk<T: HasSupport>(a: &T, b: &T) -> bool {
    ...
    loop {
        ...
        //7
        sup = get_cso_support(a,b, newdir);
        ...
        match newsimp {
            ...
            None => {
                panic!("Tried to add to a full simplex");
            }
        };
